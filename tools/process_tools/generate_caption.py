...
class TruckScenesProcessor(ProcessorTemplate):
    def __init__(self, device):
        ...
        self.scene_list = sorted([os.path.basename(d) for d in [i for i in os.listdir(args.dataset_path) if "scene-" in i]])
    ...
    def process_view_caption(self):
        ...
        for scene_name in tqdm(self.scene_list):
            scene_path = os.path.join(args.dataset_path, scene_name)
            image_paths = []
            image_names = []
            ...
            for camera in cameras:
                ...
                for img_file in glob.glob(os.path.join(camera_path, '*.png')):
    ...
    def process_scene_caption(self):
        ...
        for scene in tqdm(self.scene_list):
            ...
            # print("The text:", text, len(text.split(' ')) )
            
            # Summarize if longer than 75 words ##################### CAREFUL: Not sure how truncation works
            if len(text.split(' ')) > 75:
                sum_caption = self.summarizer(text, max_length=75, truncation=True)[0]['summary_text']
            else:
                sum_caption = text
    ...
    def process_entity_caption(self):
        ...
        view_caption_path = args.view_caption_path
        ...
        for scene in self.scene_list:
            corr_idx_file = os.path.join(args.view_caption_corr_idx_path, scene, 'view_caption_corr_idx.pkl')
        ...
        output_file = os.path.join(args.output_dir, f'caption_entity_{args.dataset}_{args.caption_model.split("/")[-1]}_{args.tag}.json')
...
if __name__ == '__main__':
    ...
    elif args.dataset == 'truckscenes':
        processor = TruckScenesProcessor(device)
    else:
